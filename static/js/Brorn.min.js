...
function json_to_input(class_name,json,set_by_name=false){
	var class_object=$CLASS(class_name);
	for (var i = 0; i < class_object.length; i++) {
		var key = "NULL";
		var val = "NULL";
		if(set_by_name){
			key = class_object[i].name;
		}
		else{
			key = class_object[i].id;
		}

		if(class_object[i].type=="checkbox"){
			class_object[i].checked = json[key];
		}
		else{
			class_object[i].value =  json[key];
		}
	}
}
