...
def dcfexport_data():
	if request.method == 'POST':
		...
		if export_type == 'form1export':
			def form1export():
				if request.method == "POST":
					query= db.select("SELECT form_1_rcus,form_1_name_dip,form_1_anchor_firm,form_1_size_of_anchor,form_1_msmes,form_1_scope_provinces,form_1_commodity,form_1_for_development,form_1_finalized_approved,form_1_date_of_parallel_review,form_1_date_of_submission,form_1_date_of_rtwg,form_1_date_of_npco_cursory,form_1_date_of_uploading_to_ifad,form_1_date_of_approval_to_ifad,form_1_date_of_ifad_no_inssuance,form_1_totalmsme,form_1_total_farmerbene,form_1_totalfo,form_1_totalhectarage_cov,form_1_hect_rehab,form_1_total_cost_rehab,form_1_hect_exp,form_1_cost_exp,form_1_euqipment,form_1_Facilities_warehouse,form_1_totalcost_prodinvest,form_1_total_rehab,form_1_total_exp,form_1_totalcost_prodinvest2,form_1_total_matching_grant,form_1_organizational,form_1_technical_trainings,form_1_post_production,form_1_others,form_1_total_capbuild,form_1_supply_chain_manager,form_1_y,form_1_ac,form_1_ad,form_1_totalproject_cost,form_1_fmi,form_1_fmi_kms FROM dcf_prep_review_aprv_status")
					...
					new_column_names = 'RCUs, Name of DIP, Anchor Firms, Size of Anchor Firm, MSMEs, Scope/Provinces, Commodity, For development (indicate date), Date: Full BPs and DIPs Finalized/Approved by RCUs and endorsed to NPCO for Review, Date of the Parallel Review with NPCO/RGMS-IFAD-RTWG, Date of submission of the  revised DIPs based on the comments from the parallel review, Date of RTWG Approval, Date: NPCO cursory review of RCUs compliance to parallel review comments and DIP  endorsement to NOTUS Uploading, Date: Uploading to IFAD NOTUS c/o NPCO, Date: IFAD Approval/No Objection Issuance, Date: NPCO Date of Approval, Total # of MSMEs, Total # of Smallholder Farming Households, Total # of FOs, Total Hectarage Covered, Hectares for Rehab, Total Cost of Rehab, Hectares for Expansion, Total Cost of Expansion, Equipment, Facilities/Warehouse, Total Cost of Productive Investments, Total cost of Rehab, Total Cost of Expansion, Total Cost of Productive Investments, Total Matching Grant, Organizational, Technical Trainings, Post-Production, Others, Total Capacity Building, Supply Chain Manager,Total Matching Grant, Total Capacity Building, Supply Chain Manager, Total Project Cost, FMI, FMI Kms'
			...
			def form2export():
				if request.method == "POST":
					...
					new_column_names = 'RCUs, PCUs, Commodity, DIP Alignment, Name of Owner or Manager of the anchor Firm/MSMEs, Sex of the Owner or Manager of the anchor Firm/MSMEs, Sector of Owner or manager of the anchor firm/MSMEs, Business Address of Owner or manager of the anchor firm/MSMEs, Name of Partner FOs engaged (Full Official Name with Abbreviation), Chairperson or Manager of Partner FO, Sex of the Chairperson or Manager of Partner FO, Sector of Chairperson or Manager of Partner FO, Office Address/ Province of FO, Total number of FO members, Total number of FO members by Sex (Male), Total number of FO members by Sex (Female), Total number of FO members by Sector (PWD), Total number of FO members by Sector (Youth), Total number of FO members by Sector (IP),Total number of FO members by Sector (SC),	Hectares Covered, Date of CPA Signing, CPA Expiration Date, Days Remaining, Date Renewed, Notable CPA incentives (Optional entry), Remarks/Status, Activity/Agreements (Outputs vis-à-vis signed CPA), Date conducted/implemented'
					new_column_names_list = new_column_names.split(',')
					df.columns = new_column_names_list
			...
			def form4export():
				if request.method == "POST":
					...
					new_column_names = 'Implementing Unit, Activity Title, Types of Training, Topic Of Training, DIP approved alignment, Name of DIPs, ACTIVITY DURATION (start date), ACTIVITY DURATION (end date), Total Number of Participants, Commodity, Venue, Name of Resource Person/Facilitator/BDSP (First Name Middle Name Last Name), RAPID Actual Budget Actual (CY 2022 Onwards e.g. 34000.00),Name of Partner/Organization, Counterpart Amount(monetize & estimates), Male, Female, Total, PWD, Youth, IP, SC, Total, Pre-Test, Post-Test, Rating, Comments/ Areas of Improvement'
					new_column_names_list = new_column_names.split(',')
					df.columns = new_column_names_list
